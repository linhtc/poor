...
class MY_Controller extends CI_Controller {
    ...
    public function checkPermission_distributor($permission = 'r') {
        $is_sign_in = $this->checkSignIn_distributor();
        if ($is_sign_in != TRUE) {
            redirect(base_url());
        }
    }
    public function checkSignIn_distributor() {
        $CI = & get_instance();
        if ($CI->session->userdata('distributor_user_id') && $CI->session->userdata('distributor_user_username')) {
            return TRUE;
        } else {
            return FALSE;
        }
    }
    public function checkPermission_deny($list) {
        $CI = & get_instance();
        $distributor_user_id = $CI->session->userdata('distributor_user_id');
        if (empty($distributor_user_id) || in_array($distributor_user_id, $list)) {
            redirect(base_url() . "");
        }
    }
    public function checkPermission_distributor_code_coupon($permission = 'r') {
        $CI = & get_instance();
        if ($CI->session->userdata('distributor_user_id') && $CI->session->userdata('distributor_user_username') && $CI->session->userdata('coupon_codes_id')) {
            return TRUE;
        } else {
            redirect(base_url());
        }
    }
    public function checkPermissionCampaigns($id) {
        if ($this->session->userdata('campaign_distributor_'.$id) != 1) {
            redirect(base_url() . "dai-ly/su-kien");
        }
        // Load model User
        $this->load->model('campaigns/Campaign');
        $this->model_name = 'Campaign';

            
        $campaign_modems = $this->Campaign->find('first', array(
            'select' => array('time_start', 'time_end'),
            'where' => array('id' => $this->session->userdata('campaign_id')),
            'order' => array(
                'id' => 'desc'
            ),
        ));
        ...
        if(strtotime($datetime_now) < strtotime($campaign_modems->time_start)){
            redirect(base_url() . "dai-ly/su-kien");
        }elseif(strtotime($datetime_now) > strtotime($campaign_modems->time_end)){
            redirect(base_url() . "dai-ly/su-kien");
        }
    }
    function check_permission_bk($class, $function){
        $CI = & get_instance();
...
//        print_r(json_encode($childrenList->{7})); exit;
