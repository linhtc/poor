...
class Profiles extends MY_Controller {
    ...
    public function sign_in() {
        ...
        $this->layout->set_layout('landing_page');

//        define('SITE_KEY',   '6Lcg7CYTAAAAANfCl1OrH9c3WQ0vbifZOHNoo9RI');
//        define('SECRET_KEY', '6Lcg7CYTAAAAAPezo_HdMX0nm6__UOpCN14ju5GQ');
        define('SITE_KEY',   '6LetLQwUAAAAAPEAm_rKz0xl4iHD7kHFy4qm1Vq2');
        define('SECRET_KEY', '6LetLQwUAAAAAAlr_YaMvnPMxOETfjyt58x9olye');

        $data = array();

        if ($this->checkSignIn() == TRUE) {
			// echo 1; exit;
            //redirect(base_url() . "admin/campaign_gifts/view");
            $administrator = $this->session->userdata('administrator');
            $permission = $this->session->userdata('user_group_permission');
            if($administrator || !empty($permission['exchanges']['view'])){
                redirect(base_url() . "admin/exchanges/view?campaign=10");
            } else{
...
//        $data['recaptcha_html'] = recaptcha_get_html(SITE_KEY, $error);
                ...
                if($resp->is_valid){*/
                    ...
                    if(count($user) > 0){
                        ...
                        if ($hashpass->checkPassword($user['password'], $data_user_post['password']) == TRUE) {
                            ...
                            /*$groupObj = $this->Group->find('first_array', array(
                                'select' => array('id','permission','locked'),
                                'where' => array('id' => $user["group_id"])
                            ));*/
                            ...
                            if(!empty($user["locked"]) || !empty($groupObj["locked"])){
                                ...
                                $this->session->set_userdata('user_menu', $this->gen_menu(!empty($user['menus_view']) ? $user['menus_view'] : $groupObj['menus_view']));
...
//                                redirect(base_url() . "admin/admins");
                ...
                /*}else{
                    $data['error_submit'] = $resp->error;
                }
            }*/
        }

        $this->parser->parse("admin/users/sign_in", $data);
    }
...
//			$permission = $this->check_permission($this->class, 'execute');
    ...
    public function views() {
        ...
        $query = "
			SELECT u.*, (SELECT g.group_name FROM mbs_groups g WHERE g.id = u.group_id) group_name
			FROM users u;
		";
