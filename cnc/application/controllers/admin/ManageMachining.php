 ...
 * @version beta.0.1
 */
class ManageMachining extends MY_Controller {
    ...
    public function edit($id='') {
        ...
        $item = $this->db->select('id, class, photo, caption')
    ...
    public function delete() {
        ...
        if(!empty($id)){
            ...
            $result = $this->db->where('id', $id)->update($this->sliderModel, $pullClass);
            echo $result; exit;
        }
        echo 0; exit;
    }
    ...
    public function data() {
        ...
        $sortMaps = array('id', 'modified', 'ipaddress', 'photo', 'caption');
        ...
        $query = "select `id`, `modified`, `ipaddress`, `photo`,  `caption` from ".$this->sliderModel;
        $query .= $this->criteria();
...
//        $query .= " order by u.id desc ";
    ...
    public function photo() {
        ...
        $file = $id.time().'.'.$extension;
        $file = urlencode($file);
        if (move_uploaded_file($fileLocation, MEDIAPATH. 'images/sliders/'.$this->folder.'/'.$file)) {
        	$url = base_url().'media/images/sliders/'.$this->folder.'/'.$file;
            $pullClass = array(
                'tmp_photo' => $url,
...
//             $response->url = $url.'?t='.time();
    ...
    private function criteria(){
        ...
        $criteria = " where `deleted` = 0 and `class` = '$this->folder' ";
    ...
    private function modify(){
        $id = $this->input->post('id', false);
        $photo = $this->input->post('photo', false);
        $caption = $this->input->post('caption', false);
        if(!empty($photo)){
            $query = "
                INSERT INTO `".$this->sliderModel."` (".(!empty($id) ? '`id`, ' : '')."`created`, `modified`, `photo`, `caption`, `class`) 
                VALUES (".(!empty($id) ? $id.', ' : '')."NOW(), NOW(), '".addslashes($photo)."', '".addslashes($caption)."', '$this->folder')
                ON DUPLICATE KEY UPDATE `photo` = VALUES(photo), `caption` = VALUES(caption), `deleted` = VALUES(deleted), modified = VALUES(modified)
                ;
            ";
