{if !empty($list)}
    {foreach from=$list key=i item=item}
        <tr>
            ...
            <td class="photo-list" style="background-color: #fff; background-image:url({$item.photo});" onclick="viewLargerPhoto(this);"></td>
            <td style="max-width:300px; overflow-x: hidden;">
				<a title="{lang('caption')}" onclick="viewFullCaption(this);" style="cursor:pointer;">{$item.caption}</a>
			</td>
