{if !empty($list)}
    {foreach from=$list key=i item=item}
        <tr>
            ...
            <td>{$item.email}</td>
            <td>
				<a title="{lang('embed')}" target="_blank" href="{$item.embed}" style="cursor:pointer;">{lang('view')}</a>
			</td>
            <td>
				<a title="{lang('place')}" target="_blank" href="{$item.place}" style="cursor:pointer;">{lang('view')}</a>
			</td>
